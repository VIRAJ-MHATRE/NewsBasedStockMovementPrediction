...
def load_xgb_close_regg():
    with open(os.path.join(MODEL_DIR, 'xgb_regressor_close.pkl'), 'rb') as file1:
        return pickle.load(file1)

def load_xgb_trade_class():
    with open(os.path.join(MODEL_DIR, 'XGB_trade_classifier.pkl'), 'rb') as file2:
...
# Close Price Gauge
    with col1:
        ...
        delta={'reference': 0, 'position': 'top' , 'suffix': ' %'}  # Optional delta display
    ))
